...
def screen():
    ...
    txt_tittle = Label(
        Windows, text="Recursos Humanos", background="white", font=("Arial", 32)
    )
    ...
    inbox_logout = Entry(Windows, font=("Arial", 15), background="white")
    ...
    button_logout = Button(
        Windows, text="Entrar", font=[16], background="blue", fg="white", width=20
    )
    button_logout[
        "command"
    ] = lambda login=inbox_logout, senha=inbox_password: open_rh_panel_one(
        Windows, login.get(), senha.get()
    )
