...
def pay_payslip(Windows1):
    ...
    txt_name = Label(Windows, text='Nome do Funcionário', font=('Arial', 20), background='white')
    ...
    txt_month= Label(Windows, text='Mês do lançamento', font=('Arial', 20), background='white')
    ...
    def browse_image():
        ...
        if file_path:
            ...
            entry_image_path.insert(0, '/') # Separating the directory/path with the file name.
    ...
    entry_image_path = tk.Entry(Windows, font=('Arial',15), background='white',width=30)
    ...
    button_exe["command"] = lambda   month = inbox_month, name_user = inbox_name, image = entry_image_path: insert(month.get(), name_user.get(), image.get())
...
def insert(month, name_user, image):
    resultado =banco_server.sql_query(f"""SELECT count(*) FROM tb_funcionarios WHERE nome_completo = '{name_user.upper()}'""")
    flag = False
    if resultado[0][0] == 1:
        flag = True
        if flag == True:
            banco_server.sql_server_insert(f"""
    ...
    """)
            messagebox.showinfo("Aviso!","Informações do holerite lançado no sistema com sucesso!")
    else:
