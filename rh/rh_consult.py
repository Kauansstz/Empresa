...
def select(Windows1):
    result = banco_server.sql_query(f"""SELECT * FROM tb_funcionarios """)
    ...
    if result[0][0] == 1:
        ...
        table = ttk.Treeview(Windows, selectmode="extended", columns=("ID","Nome", "Email", "Mátricula", "Usuário","Senha","Cargo", "Setor","Loja"))
        ...
        button_advence= Button(Windows, text="Alteração de Dados", background='blue', fg='white', font=('Arial', 18), width=20)
        button_advence["command"]= lambda button_advence=button_advence: open_alter(Windows)
        button_advence.place(x=500, y= 400)
        button_back= Button(Windows, text="Voltar", background='blue', fg='white', font=('Arial', 18), width=20)
