...
def info(Windows1):
    ...
    txt_tittle = Label(Windows, text="Preencha o Formulário", background="white", font=('Arial', 20))
    ...
    txt_name_column = Label(Windows, text="Nome da Coluna", background="white", font=('Arial', 20))
    ...
    txt_info = Label(Windows, text="Nova Informação", background="white", font=('Arial', 20))
    ...
    button_exe["command"] = lambda  name_column=inbox_name_column, new_info=inbox_info, name_user = inbox_user,: user_alter(name_column.get(), new_info.get(), name_user.get())
...
def user_alter(name_column , new_info, name_user ):
    banco_server.sql_server_insert(f"""UPDATE tb_funcionarios
